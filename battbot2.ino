...
int x = 100;//arm servo position
int val_eject = 10;//eject hatch position
int mhp = 80;//mobile_hatch_position
int sensor1,sensor2,sensor3;
int aa=0;//initialize AA and AAA counter
int aaa=0;
int vf_close=80; //val_flush
int vf_med=50; //val_flush
int vf_open=20; //val_flush
int tube1_flush = 1;
int tube2_flush = 1;
...
void setup() 
{
  ...
  Robot.beginTFT();
  flush_batt.attach(TKD0);
  eject.attach(TKD1);
  tube_hatch.attach(TKD2);
...
void release_batt()//AA and AAA batteries are released from selector
     {
     for(mhp=80;mhp<130;mhp++)
      {
       mobile_hatch.write(mhp);
       delay(40);
      }
     for(mhp=130;mhp>=80;mhp--)
      {
       mobile_hatch.write(mhp);
       delay(40);
      }
